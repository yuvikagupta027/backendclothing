...
export default function Bestsellingpage() {
    ...
    const filteredProducts = products.filter(product => {
        ...
        const matchesSize = selectedSizes.length === 0 || (Array.isArray(product.Size) && product.Size.some(size => selectedSizes.includes(size)));
        const matchesBestselling = product.Status && product.Status.includes("bestselling");

        return matchesCatname && matchesPrice && matchesSize && matchesBestselling;
    });
        ...
        <>
            <div className="container-fluid shop-page p-0">
                ...
                <main className="row m-0">
                    {/* Sidebar Filters */}
                    <aside className="col-lg-2 col-md-3 col-sm-3 d-none d-md-block bg-light p-4 filter-sidebar">
                            ...
                            <div className="mb-3">
                                ...
                                {categories.map((row) => (
                                    <div key={row._id}>
                                        ...
                                        />
                                        &nbsp;<label htmlFor={"mobile-" + row.Catname}>{row.Catname}</label>
                                    </div>
                                ))}
                            </div>
                            <div className="mb-3">
